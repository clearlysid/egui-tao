//! [`egui`] bindings for [`tao`](https://github.com/tauri-apps/tao).
//!
...
// use tao::{window::CursorGrabMode, WindowButtons, WindowLevel};
...
/// Handles the integration between egui and a tao Window.
///
...
impl State {
        ...
        match event {
            ...
            WindowEvent::Touch(touch) => {
                ...
                let consumed = match touch.phase {
                    ...
                    | tao::event::TouchPhase::Cancelled => self.egui_ctx.wants_pointer_input(),
                    tao::event::TouchPhase::Moved => self.egui_ctx.is_using_pointer(),
                    _ => todo!("some unsupported touchphase"),
                };
                EventResponse {
                    repaint: true,
                    consumed,
            ...
            WindowEvent::ReceivedImeText(ime) => {
                ...
                //         }
                //     }
                //     tao::event::Ime::Preedit(text, Some(_cursor)) => {
                //         self.ime_event_enable();
                //         self.egui_input
                //             .events
                ...
                //     }
                //     tao::event::Ime::Disabled | tao::event::Ime::Preedit(_, None) => {
                //         self.ime_event_disable();
                //     }
                // };
            ...
            WindowEvent::KeyboardInput {
                ...
                // tao/winit generate fake "synthetic" KeyboardInput events when the focus
                ...
                if *is_synthetic && event.state == ElementState::Pressed {
                    ...
                    let consumed = self.egui_ctx.wants_keyboard_input()
                        || event.logical_key == tao::keyboard::Key::Tab;
                    EventResponse {
                        repaint: true,
                        consumed,
                    }
            ...
            WindowEvent::ThemeChanged(tao_theme) => {
                self.egui_input.system_theme = Some(to_egui_theme(*tao_theme));
                EventResponse {
                    repaint: true,
                    consumed: false,
                }
            ...
            | WindowEvent::AxisMotion { .. } => EventResponse {
                repaint: false,
                consumed: false,
            },
    ...
    fn on_cursor_moved(&mut self, window: &Window, pos_in_pixels: tao::dpi::PhysicalPosition<f64>) {
    ...
    fn on_touch(&mut self, window: &Window, touch: &tao::event::Touch) {
        let pixels_per_point = pixels_per_point(&self.egui_ctx, window);
        ...
        self.egui_input.events.push(egui::Event::Touch {
            ...
            phase: match touch.phase {
                ...
                _ => todo!("some unsupported touch phase"),
            },
            pos: egui::pos2(
            ...
            force: match touch.force {
                Some(tao::event::Force::Normalized(force)) => Some(force as f32),
    ...
    fn on_mouse_wheel(&mut self, window: &Window, delta: tao::event::MouseScrollDelta) {
        let pixels_per_point = pixels_per_point(&self.egui_ctx, window);
    ...
    fn on_keyboard_input(&mut self, event: &tao::event::KeyEvent) {
        ...
        // let physical_key = if let tao::keyboard::PhysicalKey::Code(keycode) = *physical_key {
        //     key_from_key_code(keycode)
        // } else {
        //     None
        // };

        let physical_key = key_from_key_code(*physical_key);
        let logical_key = key_from_tao_key(tao_logical_key);
        ...
        //     .as_ref()
        //     .map(|t| t.as_str())
        //     .or_else(|| tao_logical_key.to_text())
        // {
    ...
    fn set_cursor_icon(&mut self, window: &Window, cursor_icon: egui::CursorIcon) {
        ...
        if is_pointer_in_window {
            ...
            if let Some(tao_cursor_icon) = translate_cursor(cursor_icon) {
                window.set_cursor_visible(true);
                window.set_cursor_icon(tao_cursor_icon);
            } else {
...
fn to_egui_theme(theme: tao::window::Theme) -> Theme {
    match theme {
...
pub fn update_viewport_info(
    ...
    viewport_info.focused = Some(window.is_focused());
}
...
fn translate_mouse_button(button: tao::event::MouseButton) -> Option<egui::PointerButton> {
    match button {
        ...
        // tao::event::MouseButton::Back => Some(egui::PointerButton::Extra1),
        // tao::event::MouseButton::Forward => Some(egui::PointerButton::Extra2),
        tao::event::MouseButton::Other(_) => None,
        _ => todo!("unsupported mouse button"),
    }
}

fn key_from_tao_key(key: &tao::keyboard::Key) -> Option<egui::Key> {
    match key {
        tao::keyboard::Key::Character(str) => egui::Key::from_name(str),
        tao::keyboard::Key::Unidentified(_) | tao::keyboard::Key::Dead(_) => None,
        &_ => key_from_named_key(key.clone()),
        // &_ => todo!("unsupported key from tao key"),
    }
}

fn key_from_named_key(named_key: tao::keyboard::Key) -> Option<egui::Key> {
    use egui::Key;
    use tao::keyboard::Key as NamedKey;
...
fn key_from_key_code(key: tao::keyboard::KeyCode) -> Option<egui::Key> {
    use egui::Key;
...
fn process_viewport_command(
    ...
    match command {
        ...
        ViewportCommand::InnerSize(size) => {
            ...
            window.set_inner_size(requested_size);
        ...
        ViewportCommand::WindowLevel(l) => match l {
        ...
        ViewportCommand::Icon(icon) => {
            let winit_icon = icon.and_then(|icon| to_tao_icon(&icon));
        ...
        ViewportCommand::IMEPurpose(p) => {
            // window.set_ime_purpose(match p {
            //     egui::viewport::IMEPurpose::Password => tao::window::Ime ImePurpose::Password,
            //     egui::viewport::IMEPurpose::Terminal => tao::window::ImePurpose::Terminal,
            //     egui::viewport::IMEPurpose::Normal => tao::window::ImePurpose::Normal,
            // })
        }
        ...
        ViewportCommand::RequestUserAttention(a) => {
            window.request_user_attention(match a {
                ...
                egui::UserAttentionType::Critical => Some(tao::window::UserAttentionType::Critical),
        ...
        ViewportCommand::SetTheme(t) => window.set_theme(match t {
            egui::SystemTheme::Light => Some(tao::window::Theme::Light),
            egui::SystemTheme::Dark => Some(tao::window::Theme::Dark),
            egui::SystemTheme::SystemDefault => None,
        }),
        ViewportCommand::ContentProtected(v) => window.set_content_protection(v),
        ...
        ViewportCommand::CursorGrab(o) => {
            if let Err(err) = window.set_cursor_grab(match o {
                egui::viewport::CursorGrab::None => false,
                egui::viewport::CursorGrab::Confined => true,
                egui::viewport::CursorGrab::Locked => true,
            }) {
                log::warn!("{command:?}: {err}");
            }
        }
...
pub fn create_window<T>(
    ...
    let window_attributes = create_tao_window_attributes(egui_ctx, viewport_builder.clone());

    let window = tao::window::Window::new(event_loop).expect("couldn't create window");
    // let window = event_loop.create_window(window_attributes)?;
    apply_viewport_builder_to_window(egui_ctx, &window, viewport_builder);
    Ok(window)
}

pub fn create_tao_window_attributes(
    egui_ctx: &egui::Context,
    viewport_builder: ViewportBuilder,
    ...
    let mut window_attributes = tao::window::WindowAttributes::default();

    window_attributes.title = title.unwrap_or_else(|| "egui window".to_owned());

    window_attributes.transparent = transparent.unwrap_or(false);
    window_attributes.decorations = decorations.unwrap_or(true);
    window_attributes.resizable = resizable.unwrap_or(true);
    window_attributes.visible = visible.unwrap_or(true);
    window_attributes.maximized = maximized.unwrap_or(false);
    // window_attributes.active = active.unwrap_or(true);
    ...
    // })
    // .with_fullscreen(
    //     fullscreen.and_then(|e| e.then_some(tao::window::Fullscreen::Borderless(None))),
    // )
    ...
    if let Some(icon) = icon {
        ...
        window_attributes.window_icon = tao_icon
    }
    ...
    #[cfg(target_os = "macos")]
    {
        // use tao::platform::macos::WindowAttributesExtMacOS as _;
        // window_attributes = window_attributes
...
fn to_tao_icon(icon: &egui::IconData) -> Option<tao::window::Icon> {
    if icon.is_empty() {
        None
    } else {
        profiling::function_scope!();
        match tao::window::Icon::from_rgba(icon.rgba.clone(), icon.width, icon.height) {
...
pub fn apply_viewport_builder_to_window(
    ...
    if let Some(mouse_passthrough) = builder.mouse_passthrough {
        window
            .set_ignore_cursor_events(!mouse_passthrough)
...
pub fn short_device_event_description(event: &tao::event::DeviceEvent) -> &'static str {
...
/// Useful for logging and profiling.
pub fn short_window_event_description(event: &tao::event::WindowEvent) -> &'static str {
    use tao::event::WindowEvent;
